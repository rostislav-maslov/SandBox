...
<div class="row">
    <form action="<%= CategoryAdminRoutes.ALL%>" method="GET">
        <div class="col-lg-5">
            ...
            <table class="table table-bordered" id="table-1">
                ...
                <c:forEach items="${search.result}" var="doc">
                    <tr>
                        <td>${doc.title}</td>
                        <td>
                            <c:if test="${not empty doc.parentId}">
                                ${categories.get(doc.parentId).title}
                            </c:if>
                        </td>
                        <td>${doc.sortNum}</td>
                        <td>${doc.available}</td>
                        ...
                        <td>
                            <c:url value="<%= CategoryAdminRoutes.EDIT%>" var="editUrl">
                            ...
                            <c:url value="<%= CategoryAdminRoutes.DELETE%>" var="urlDelete">
                                <c:param name="id" value="${doc.id}"/>
                            </c:url>
...
<div class="row">
    <div class="col-lg-12 text-center">
        <ul class="pagination pagination-sm">
            <c:url value="<%= CategoryAdminRoutes.ALL%>" var="urlPrev">
            ...
            <c:forEach items="${search.paginator()}" var="page">
                <c:url value="<%= CategoryAdminRoutes.ALL%>" var="urlPage">
                    <c:param name="query" value="${search.query}"/>
            ...
            <c:url value="<%= CategoryAdminRoutes.ALL%>" var="urlNext">
                <c:param name="query" value="${search.query}"/>
